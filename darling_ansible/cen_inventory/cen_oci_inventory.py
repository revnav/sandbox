...
class OCIInventory:
    ...
    def __init__(self):
        ...
        try:
            ...
            ##Added New AnalyticsClient
            self._analytics_clients = dict(
                (region, self.create_service_client(AnalyticsClient, region=region))
                for region in self.regions
            )
            ##Added New AnalyticsClient

            ##Added New AnalyticsClient
            self._integration_clients = dict(
                (region, self.create_service_client(IntegrationInstanceClient, region=region))
                for region in self.regions
            )
            ##Added New AnalyticsClient
    ...
    #ADDED NEW for database clients
    ...
    #ADDED NEW for analytics clients
    def get_analytics_client_for_region(self, region):
        if region not in self._analytics_clients:
            raise ValueError(
                "Could not fetch the analytics client for region {0}.".format(region)
            )
        return self._analytics_clients[region]
    #ADDED NEW for analytics clients

    #ADDED NEW for integrations clients
    def get_integrations_client_for_region(self, region):
        if region not in self._integration_clients:
            raise ValueError(
                "Could not fetch the integrations client for region {0}.".format(region)
            )
        return self._integration_clients[region]
    #ADDED NEW for integrations clients
    ...
    def get_instances(self, compartment_ocids):
        ...
        for region in self.regions:
            ...
            list_service = database_client.list_autonomous_databases
            list_services.add(list_service)
            list_service = database_client.list_autonomous_container_databases
            list_services.add(list_service)
            list_service = database_client.list_autonomous_data_warehouses
            list_services.add(list_service)
            list_service = database_client.list_db_systems
            list_services.add(list_service)
            list_service = database_client.list_vm_clusters
            list_services.add(list_service)
            list_service = database_client.list_autonomous_exadata_infrastructures
            list_services.add(list_service)
            analytics_client = self.get_analytics_client_for_region(region)
            list_service = analytics_client.list_analytics_instances
            list_services.add(list_service)
            integrations_client = self.get_integrations_client_for_region(region)
            list_service = integrations_client.list_integration_instances
            list_services.add(list_service)
        ...
        if self.params["enable_parallel_processing"]:
            for region in self.regions:
                ...
                #Added NEW for database clients
                with self.pool(processes=num_threads) as pool:
                    get_custom_filtered_instances_for_region = partial(
                        self.get_custom_filtered_instances, region=region,list_service=list_service
                    )
                    lists_of_instances = pool.map(
                        get_custom_filtered_instances_for_region, compartment_ocids
                    )
                for sublist in lists_of_instances:
                    instances[region].extend(sublist)
            ...
            for region in self.regions:
                for compartment_ocid in compartment_ocids:
                    ...
                    for list_service in list_services:
                        instances[region].extend(
                            self.get_custom_filtered_instances(compartment_ocid, region,list_service)
                        )
    ...
    #Added New
    def get_custom_filtered_instances(self, compartment_ocid, region,list_service):
        try:
    ...
    def build_inventory_for_instance(self, instance, region):
        ...
        try:
            ...
            #print("Now printing instance vars")
            #print(instance_vars)

            common_groups = set(["all"])
            #Added New if condition
            if 'availability_domain' in instance_vars:
            ...
            #Added New if condition
            if 'defined_tags' in instance_vars:
            ...
            #Added New if condition
            if 'extended_metadata' in instance_vars:
            ...
            #Added New
            vnic_record_found=False
            for vnic_attachment in vnic_attachments:
                #print("Vnic found")
                vnic_record_found=True
            ...
            #Added NEW
            if not vnic_record_found:
                customhostname = region + "_" + type(instance).__name__
                groups = set(common_groups)
                self.create_instance_inventory_for_host(
                    instance_inventory,
                    customhostname,
